...
const seed_admin_data_1 = require("./data/seed-admin.data");
async function seedAdmin(dataSource) {
    const repository = dataSource.getRepository(user_entity_1.User);
    const data = (0, seed_admin_data_1.getAdminData)();
    const password = await bcrypt.hash(data.password, Number(process.env.HASH_SALT) || 10);
